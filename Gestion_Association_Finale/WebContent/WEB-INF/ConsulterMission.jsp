<%@ page pageEncoding="UTF-8" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
        <title>Index</title>
       <link type="text/css" rel="stylesheet" href="<c:url value="/assets/css/bootstrap.css"/>" />
       <link type="text/css" rel="stylesheet" href="<c:url value="/assets/css/bootstrap_badis.css"/>" />
        <link type="text/css" rel="stylesheet" href="<c:url value="/assets/css/bootstrap.min.css"/>" />
       <link type="text/css" rel="stylesheet" href="<c:url value="/assets/font-awesome/css/font-awesome.min.css"/>" />
       <link type="text/css" rel="stylesheet" href="<c:url value="/assets/js/bootstrap.js"/>" />
       <link type="text/css" rel="stylesheet" href="<c:url value="/assets/js/bootstrap.min.js"/>" />
        <link type="text/css" rel="stylesheet" href="<c:url value="/assets/js/jquery-1.10.2.min.js"/>" />
             <link type="text/css" rel="stylesheet" href="<c:url value="/assets/css/customize-template.css"/>" />
      
       
    </head>
    <body>
    
    <nav class="navbar navbar-inverse" role="navigation">

<h3><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Gestion des Missions: <samp>  Page de consultation d'une mission</samp>
<a href="<c:url value="deconnexion"/>" class="btn btn-link" id="droite" >Deconnexion</a> 

</strong> </h3>


</nav>
    
    <div class="container">
    
    
    
 <div class="row">

 <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
           
           
            <c:import url="/menu.jsp" />
           
        </div>
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
            <div class="listing listing-danger">
                <div class="shape">
                    <div class="shape-text">Infos</div>
                </div>
                <div class="listing-content">
                   <h3 id="tt">Votre Login: ${sessionScope.login}</h3>
                   
                    
                </div>
            </div>
        </div>
</div><!-- row -->

</br>
</br>



<div class="row">

<div class="col-md-1"></div>
  
         <div class="col-md-10">
         
          <div class="box pattern pattern-sandstone">
        
           <h5 id="titre3"><i class="icon-table">  Table des missions  </i></h5> 
           
                            <div class="box-content box-table">
                                <table id="sample-table" class="table table-hover table-bordered tablesorter">
                                    <thead>
                                        <tr>
                                            <th>ID Mission</th>
                                            <th>Nom Mission</th>
                                            <th>Description</th>
                                             <th>Lieu</th>
                                              <th>Date</th>
                                              <th>Budget</th>
                                              <th>Nom admin</th>
                                              <th>Nom Chef</th>
                                             
                                              
                                            <th>Action</th>
                                            
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <c:forEach var="i" begin="0" end="${tableau.size()-1}">
                                        <tr>
                                            <td><c:out value="${tableau[i].idMission}"/></td>
                                            <td><c:out value="${tableau[i].nomMission}"/></td>
                                            <td><c:out value="${tableau[i].description}"/></td>
                                            <td><c:out value="${tableau[i].lieuMission}"/></td>
                                            <td><c:out value="${tableau[i].dateMission}"/></td>
                                            <td><c:out value="${tableau[i].budget}"/></td>
                                            <td><c:out value="${tableau[i].administrateur.nom}"/></td>
                                            <td><c:out value="${tableau[i].chef.nom}"/></td>
                                           
                                              <td class="td-actions">
                                              
                                  <div class="row">       
                                              <div class="col-md-2"></div>      
                                             <div class="col-md-3"> 
                                               <form method="get" action="Servlet_modifier_mission">
                                                   
                                                    <%--cette input est invisible grace a hidden pour monter l'id de produit a modofier --%>
                                                   <input type="hidden" name="id" value="${tableau[i].idMission}"/>
                                                   <input  type="submit" value="M" class="btn btn-small btn-info"/>
                                                      
                                             </form>
                                                <!--  <a href="<c:url value="Servlet_modifier_mission"/>"  type="button" class="btn btn-small btn-info">
                                                    <i><strong>M</strong></i>
                                                </a>-->

                                               </div><!-- col-md-3 -->
                                               <div class="col-md-3">
                                               <form method="post" action="Servlet_supprimer_mission">
                                                   
                                                    <%--cette input est invisible grace a hidden pour monter l'id de produit a supprimer --%>
                                                   <input type="hidden" name="id" value="${tableau[i].idMission}"/>
                                                   <input  type="submit" value="S" class="btn btn-small btn-danger"/>
                                                      
                                                </form>
                                                <!--  <a href="<c:url value="Servlet_supprimer_mission"/>" type="button" class="btn btn-small btn-danger">
                                                    <i class="btn-icon-only icon-remove"></i>
                                                </a>-->
                                                </div><!-- col-md-3 -->
                                                <div class="col-md-3">
                                                  <form method="get" action="Servlet_consulter_rapport">
                                                   
                                                    <%--cette input est invisible grace a hidden pour monter l'id de produit a supprimer --%>
                                                   <input type="hidden" name="id" value="${tableau[i].idMission}"/>
                                                   <input  type="submit" value="R" class="btn btn-small btn-success"/>
                                                      
                                                </form>
                                               <!--  <a href="<c:url value="Servlet_consulter_rapport"/>" type="button" class="btn btn-small btn-success">
                                                    <i><strong>R</strong></i>
                                                </a>-->
                                                </div><!-- col-md-3 -->
                                                  <div class="col-md-1"></div> 
                                            </td>
                                           
                                        </tr>
                                        </c:forEach>
                                       
                                       
                                       
                                      
                                    </tbody>
                                </table>
             
                            </div>
                        </div>
                        
           <a href="<c:url value="Servlet_ajouter_mission"/>" class="btn btn-warning">Ajout Mission</a>
           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                         <a href="<c:url value="Servlet_affecter_categorie"/>" class="btn btn-warning">Affecter Miss/Cat</a>
                         <a href="<c:url value="Servlet_affecter_membre"/>" class="btn btn-warning">Affecter Mem/Miss</a>
          <a href="<c:url value="ServletAdmin"/>" class="btn btn-warning" id="droite" >tableau de bord</a>
         
         
         
         
         
         </div>

<div class="col-md-1"></div>
</div><!-- row -->


</div>
</br>
</br>

</body>
</html>